...
class DataKern:
    ...
    def mark_errors(self, column_name, test_name, error_description, error_mask, md):
        """
        ...
        post_test_df.loc[post_test_df.index.isin(
            error_indices), test_name] = error_description

        styled_df = post_test_df.style.apply(self.__color_cells, axis=None, error_indices=error_indices,
                                             column=column_name)
        styled_df.to_excel(self.file_path,
