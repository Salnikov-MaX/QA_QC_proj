...
class QA_QC_seismic(QA_QC_main):
    def __init__(self, file_path: str, license_area_poly: list = None, surfaces_path_list: list = None,
                 faults_file_path: str = None) -> None:
        """
        Args:
    ...
    def test_edge_zone_evaluation(self, get_report=True) -> dict:
        ...
        self.update_report(report_text)
        return {'variance_list': variance_list, 'split_point': split_point, 'edge_zone_mask': edge_zone_mask,
        ...
        if not self.surfaces_path_list:
            ...
            for path in self.surfaces_path_list:
                ...
                try:
                    ...
                    inside_depth_range = (map_df['Dept'] >= self.coordinate_z.min()) & (
                            map_df['Dept'] <= self.coordinate_z.max())
                    ...
                    if map_df.empty:
                        ...
                        seismic_cube_r = self.seismic_cube.reshape(-1, self.seismic_cube.shape[2])
                        indxs_0 = find_closest_indices_x_y(self.coordinate_x, self.coordinate_y, map_df.X.to_numpy(),
                                                           map_df.Y.to_numpy())
