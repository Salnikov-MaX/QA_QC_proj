...
SupportKeyWords = [
    ...
    'SW_NPSL', 'ACTNUM',
    # Custom KeyWord
    "SGAS", "SGCR", "IRR.GASSATURATION",
    "SOIL", "CRITICALOILSATURATION", "SWAT",
    "IRR.WATERSATURATION", "ABOVECONTACT",
]
...
class GRDECL_Parser:
    def __init__(self, filename='', nx=0, ny=0, nz=0, grid_type="Nan", grid_data=True):
        ...
        if (len(filename) > 0):
            self.read_GRDECL(grid_data)

        # Derived variabls
        self.CELL_FAULT = []

    ######[read_GRDECL]######
    def read_GRDECL(self, grid_data):
    ...
    def LoadVar(self, Keyword, DataArray, DataSize):
        ...
        if (Keyword in SupportKeyWords):  # KeyWords Check
            assert len(DataArray) == DataSize, '\n     [Error-%s] Incompatible data size! %d-%d' % (
                Keyword, len(DataArray), DataSize)
    ...
    def getCornerPointCellIdx(self, i, j, k):
        ...
        p7_id, p8_id = getIJK(2 * i, 2 * j + 1, 2 * k + 1, nx, ny, nz), getIJK(2 * i + 1, 2 * j + 1, 2 * k + 1, nx, ny,
                                                                               nz)
