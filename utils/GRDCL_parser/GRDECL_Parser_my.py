...
class GRDECL_Parser_my:
    def __init__(self,filename='',nx=0,ny=0,nz=0, grid_data=True, GRID_type=None, grid_dim=None):
        ...
        if(len(filename)>0):
            self.read_GRDECL(grid_data, GRID_type, grid_dim)

        #Derived variabls 
        self.CELL_FAULT=[]
    
    ######[read_GRDECL]######
    def read_GRDECL(self , grid_data=True, GRID_type=None, grid_dim=None):
        ...
        if not grid_data:
            ...
            self.NX,self.NY,self.NZ=grid_dim[0],grid_dim[1],grid_dim[2]
        ...
        for i,block in enumerate(contents_in_block):#Keyword, Block-wise
            ...
            elif(Keyword == 'ACTNUM'):  # TOP position
                assert len(DataArray) == self.N, '[Error] Incompatible TOPS data size!'
                self.ACTNUM = np.array(DataArray, dtype=float)
            elif(Keyword == 'FACIES'):  # TOP position
                assert len(DataArray) == self.N, '[Error] Incompatible TOPS data size!'
                self.FACIES = np.array(DataArray, dtype=float)
            elif(Keyword == 'PERMX'):  # TOP position
                assert len(DataArray) == self.N, '[Error] Incompatible TOPS data size!'
                self.PERMX = np.array(DataArray, dtype=float)
